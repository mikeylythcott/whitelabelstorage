...
class WpGetApi_Extras_Actions {
    ...
    public function is_form_valid( $endpoint, $api_data ) {
        ...
        // Only submit if API response contains...
        if ( $validate == 'send_if_api_response' ) {
            $is_valid = $this->recursive_array_search( $validate_val, $api_data['api_result'] ) ? true : false;
        } else 

        // Don't submit if API response contains...
        if ( $validate == 'dont_send_if_api_response' ) {
     ...
     * 
     */
    public function fluent_forms( $entryId, $formData, $form ) {
        ...
        foreach ( $endpoints as $i => $endpoint ) {
            ...
            // pass the functions args & make sure the array is array and not objects
            $args['action_args'] = $formData;
            $args['action_args']['entry_id'] = $entryId;
    ...
    public function transition_post_status( $new_status, $old_status, $post ) {
        ...
        foreach ( $endpoints as $i => $endpoint ) {
            ...
            if( 
                ( in_array( 'any', $endpoint['endpoint']['old_post_status'] ) || in_array( $old_status, $endpoint['endpoint']['old_post_status'] ) ) ||
    ...
    public function woocommerce_order_status_changed( $order_id, $old_status, $new_status, $order ) {
        ...
        foreach ( $endpoints as $i => $endpoint ) {
            ...
            // if we have the statuses we want, then do it
            if( 
                ( in_array( 'any', $endpoint['endpoint']['old_order_status'] ) || 
    ...
    public function call_endpoint( $endpoint, $argument_values, $action ) {
        ...
        $headers_value = add_filter( 'wpgetapi_header_parameters', function( $params, $api ) {
            $this->headers_value = $params;
            return $params;
        }, 9999, 2 );
        // REMINDER: body does not work with GET
        $body_value = add_filter( 'wpgetapi_final_body_parameters', function( $params, $api ) {
            $this->body_value = $params ;
            return $params;
        }, 9999, 2 );
        $query_string_value = add_filter( 'wpgetapi_query_parameters', function( $params, $api ) {
            $this->query_string_value = $params;
            return $params;
        }, 9999, 2 );
        $endpoint_value = add_filter( 'wpgetapi_endpoint', function( $endpoint, $api ) {
            $this->endpoint_value = $endpoint;
            return $endpoint;
        }, 9999, 2 );

        $response_code = add_filter( 'wpgetapi_before_retrieve_body', function( $response, $response_code, $api ) {
            $this->response_code = $response_code;
            return $response;
        }, 9999, 3 );

        $final_url = add_filter( 'wpgetapi_final_url', function( $final_url, $api ) {
    ...
    public function output_api_response( $action ) {
        add_filter( 'wpgetapi_' . $action . '_return_value', function( $null, $result, $arg_values, $values_sent, $endpoint, $response_code ) {
            return array( 
                'api_result' => $result,
