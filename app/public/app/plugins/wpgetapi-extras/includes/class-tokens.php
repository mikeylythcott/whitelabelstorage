...
class WpGetApi_Extras_Tokens {
    ...
    public function do_loop_line_items( $tokens, $action_args, $orig_params, $api ) {
        ...
        foreach( $tokens as $i => $token ) {
            ...
            if( $token == '(line_items:start)' ) {
                unset( $tokens[ $i ] );
                continue;
            }
            if( $token == '(line_items:end)' ) {
                unset( $tokens[ $i ] );
                continue;
            }
                ...
                switch ( $type ) {
                    ...
                    case 'POST':
                        ...
                        if( isset( $_POST['wpforms']['fields'] ) ) {
                            ...
                            else if( isset( $_POST['wpforms']['fields'][ $var ]['date'] ) ) {

                                $text = $_POST['wpforms']['fields'][ $var ]['date'];

                            } 
                            else if( isset( $_POST['wpforms']['fields'][ $var ] ) && is_array( $_POST['wpforms']['fields'][ $var ] ) ) {
                        ...
                        // formidable forms
                        if( isset( $_POST['frm_action'] ) && isset( $_POST['item_meta'] ) ) {
