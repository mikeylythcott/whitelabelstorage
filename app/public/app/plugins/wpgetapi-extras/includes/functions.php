...
function wpgetapi_action_log( $action = '', $arg_values = array(), $values_sent = array(), $endpoint = array(), $response_code = 200, $api_response = array() ) { 
    ...
    if ( is_array( $api_response ) )
        $api_response = var_export_short( wpgetapi_format_action_array( $api_response ) );

    if ( is_array( $values_sent ) )
        $values_sent = var_export_short( wpgetapi_format_action_array( $values_sent ) );

    if ( is_array( $arg_values ) )
        $arg_values = var_export_short( wpgetapi_format_action_array( $arg_values ) );
...
function wpgetapi_format_action_array( array $haystack ) {
    ...
    $nulls = ['', null, []];
    $sensitive = [ 'cvv', 'user_pass', 'password', 'authorization', 'apikey', 'api_key', 'clientid', 'client_id', 'clientsecret', 'client_secret' ];
