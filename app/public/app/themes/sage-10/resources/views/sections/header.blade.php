...
<header class="banner d-flex justify-content-end align-items-center">
  <a class="brand me-auto" href="{{ home_url('/') }}">
    @if($header_logo)
      ...
      <span class="text-hide d-inline-block">{{ get_bloginfo('name', 'display') }}</span>
    @else
      {!! $siteName !!}
    @endif
  </a>
  ...
  @if($pay_your_bill_title)
    <a href="{!! $bill_payment_link !!}" class="button is-icon is-white-shadowed is-small w-inline-block">
  @else
